...
- `golden_set.json` — 18 questions in three categories, each with an expected
  answer, expected Answer mode, and expected citation source(s):
...
- `run_golden.py` — runs the set through the Answer Engine and writes `results.md`.
- `results.md` — generated results table (the documented evidence for #11).
...
python -m eval.run_golden             # writes eval/results.md
```
